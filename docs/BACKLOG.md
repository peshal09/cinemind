...
- [x] **Reranking layer for "#1 = best match"** — DONE (2026-06). The Critic now **LLM-reranks**
  ...
  Inbetweeners Movie / Bring It On* (not the film *Fun*); "rainy sunday warm" → *Sleepless in
  Seattle / Notting Hill*. **Caveat:** it only reorders what retrieval surfaced — queries
  where the right films never enter the pool (e.g. "best of 2015") still need the
  constraint-aware retrieval fix below.
